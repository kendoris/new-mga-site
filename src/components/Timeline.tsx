    ...
    <div className="relative">
      ...
      <div className="space-y-8">
        {events.map((event, index) => {
          const content = (
            <div className={`bg-[var(--content-bg)] backdrop-blur-sm rounded-lg p-6 shadow-md ${event.href ? 'hover:shadow-lg transition-shadow cursor-pointer' : ''}`}>
              ...
              {event.href && (
                <span className="inline-block mt-2 text-sm text-[var(--nav-bg)] font-semibold">
                  Read more →
                </span>
              )}
            </div>
